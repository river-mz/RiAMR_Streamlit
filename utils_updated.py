...
def preprocess_data(input_df):
    ...
    pca_model_path = '/ibex/user/xiex/ide/AMR_proj2/DataPreprocessing/data_processing_ver2/tool/pca_model.joblib'
    ...
    ipca_model_path = '/ibex/user/xiex/ide/AMR_proj2/DataPreprocessing/data_processing_ver2/tool/entity_pca_model.joblib'
    ipca = joblib.load(ipca_model_path)

    filtered_entities_path = '/ibex/user/xiex/ide/AMR_proj2/DataPreprocessing/data_processing_ver2/intermedia/filtered_entities.csv'
    ...
    if not final_df.empty:  # 先检查DataFrame是否非空
        for col in final_df.columns:
            if pd.api.types.is_bool_dtype(final_df[col]):  # 使用Pandas官方推荐的类型检查方法
                final_df[col] = final_df[col].astype(int)

    with open('/ibex/user/xiex/ide/AMR_proj2/streamlit_running/feature_columns_updated.json', 'r') as file:
        feature_columns = json.load(file)
    print('Feature columns loaded:', feature_columns)
    print('Feature columns we have:',final_df.columns)

    lost_columns = set(feature_columns) - set(final_df.columns)
    print('Lost columns:', lost_columns)
    for col in lost_columns:
        final_df[col] = -1  # 或 np.nan、False，根据业务逻辑决定
